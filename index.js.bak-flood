...
async function runScan() {
  ...
  try {
    ...
    // 8. Send WhatsApp alerts for actionable signals (BUY, SELL, DANGER only)
    const actionableSignals = signals.filter(s => ['BUY', 'SELL', 'DANGER'].includes(s.type));
    if (actionableSignals.length > 0) {
      const alertMsg = formatAlertMessage(actionableSignals);
      if (alertMsg) sendWhatsApp(alertMsg);
    }
